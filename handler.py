...
try:
    ...
    # device_map="auto" is crucial for loading large models
    pipe = pipeline(
        "text-generation",
...
def enhance_prompt_with_gptoss(prompt: str, max_new_tokens: int = 300) -> str:
    ...
    # Your post-processing logic to clean the output
    # (This section is copied directly from your original code)
    unwanted_prefixes = [
    ...
    for line in lines:
        ...
        if not line:
            continue
        if any(phrase in line.lower() for phrase in ['we need to', 'must be', 'no repeating', 'so something like', 'but we need']):
            continue
